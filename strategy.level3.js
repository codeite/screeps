...
function upgradeToLevelThree(spawn) {
    ...
    route = spawn.room.findPathStatic(spawn.pos, spawn.room.controller.pos, {ignoreCreeps: true});
...
function applyLevelThree(spawn, intel, army, stratergy) {
    ...
    for(var i=0; i<2; i++) { 
      army.push({chassis: chassis.staticWorker(5), name: 'HeavyWorker'+(heavyWorkerId++), role: 'drill'});
